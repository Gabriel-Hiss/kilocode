 ...
 */
export function isDetachedHead(workspacePath: string): boolean {
	try {
 ...
 */
export function getGitHeadPath(workspacePath: string): string {
	try {
 ...
 */
export function getGitState(workspacePath: string): { branch: string; commit: string; isDetached: boolean } | null {
	try {
		const isDetached = isDetachedHead(workspacePath)
		...
		const commit = getCurrentCommitSha(workspacePath)
